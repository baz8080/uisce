...
**Beware the phantom backlog (time was wasted on this once, 2026-07-20).** A gap in `inferred_cases` does *not* mean inference hasn't run. `uisce-infer` decides its work from the **JSONL** (latest description-hash + prompt-version per case), so after an inference run the JSONL is complete but your local `inferred_cases` table still reflects whenever `uisce-build-inferred` last ran against it — querying the table by hand at that point shows a "backlog" of cases that are in fact already inferred, while `uisce-infer` correctly reports nothing to do (its cache is the JSONL, and the descriptions haven't changed). A snapshot taken that day showed 56 "never-inferred" cases this way; all 56 were in the JSONL, and rebuilding the table closed the gap. The count printed by `uisce-build-inferred` is computed *after* the rebuild, so it never shows this artifact — it reports only the real backlog: cases with no JSONL record at all, i.e. scraped since the last `uisce-infer` run. Trust the printed count, not a hand query against a possibly-stale table.
...
The one thing that had to move with it: the banner's "Updated N ago" used the build clock. Once the site can rebuild without new data, that stamp would reset on every UI push and the 24h "last build may have failed" warning — which exists to catch a dead data build — would be hidden by ordinary UI work. The banner now reads `data_as_of_iso`, which `site.py:data_horizon` takes from `MAX(cases.last_seen)`, i.e. the last time the pipeline read the feed; `generated` stays on the build clock for the footer. Same principle esb and lifts settled as "windows end at the collection horizon, not the build clock".
