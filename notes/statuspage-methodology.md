...
## The completion-vs-status contradiction is display-only (2026-09-05)

A reader flagged CAR00119809 (Carlow, Burst Water Main): its own 2:36pm update says "Works are now complete ... supply should start returning", but the county page still listed it under "Open now". `Case.is_open` (`site.py`) reads only `row["status"] == "Open"` and never consults `end_source` / `end_local_date` - the extracted completion the accrual math already treats as authoritative (`OBSERVED_END_SOURCES`) and uses to stop charging the case downtime. So the case's *hours* were already correctly closed off at the reported time; only the live badge and list still mirror the feed's `status` field, which the "20 of 508" figure just above already measured as capable of staying stale after a stated completion. This is the same shape as case 221165 above ("Do-not-consume notices got the pairing, not the exclusion"), just on an ordinary outage rather than a boil notice.

**Considered: making `is_open` also check for a passed `completion_update` end.** Prototyped and set aside. It fixes the reported case, but `is_open` feeds the open list, `open_total`, the per-event history's "still open" / "closed" text and the Atom feed all at once, and roughly a dozen existing tests build their row with `status="Open"` on the default fixture - which already carries a `completion_update` - purely as shorthand for "this case is open," with no intent to exercise this interaction. That every one of those tests currently passes is stronger evidence that the codebase treats `status` and the extracted end as independent by convention than that this exact combination was ever deliberately chosen; either reading needs a decision, not a side effect of a five-line patch.

There is also a real asymmetry in what being wrong costs, the same one that settled the do-not-consume/boil split above: hiding a case from "Open now" on a false-positive completion match (a `rules.py` template misread, or a follow-up problem the notice never got a fresh update for) hides a live disruption from someone checking whether their road is affected. Leaving a completed case visible for a build cycle or two, with its own words underneath it, costs a moment of reader confusion - which is exactly what `_county_open_html` already carries the notice text to resolve.

**Reopen this if** a labelled sample measures the false-negative rate (a genuinely open case wrongly read as complete) low enough to accept, or the 20-of-508 figure above grows enough that the display gap becomes the bigger cost. Until then, the scoped fix is: `is_open` becomes `row["status"] == "Open" and not (observed_end and reported_end <= now)`; the tests to update at minimum are `test_an_open_case_names_its_area_instead_of_being_listed_twice`, `test_an_open_case_is_never_listed_as_resolved`, `test_the_county_page_carries_it_only_for_open_notices` and `test_an_open_row_links_its_reference_on_water_ie` in `tests/test_site.py`, plus a new regression case shaped like CAR00119809.
