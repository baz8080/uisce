...
**Amended 2026-09-24 (owner decision): the close date follows the same reading.** The paragraph above kept `closed_at` as the close date so the "closed in <month>" lists and the history's "closed <date>" kept meaning "the build that saw the feed close it". The retroactive review measured what that cost: cases closed by their own text but still `Open` in the feed were in neither the open list nor any closed list, completions the feed closed three days later were filed under the following month, and every case that closed before schema v2 (July) had no close date at all, so April to June had no closed lists. `closed_on(row, now)` in `site.py` now dates a close from the notice's own reported completion where it has one, and falls back to `closed_at` only for a case with none; a completion dated before a re-stamped start also falls back, because the history reads a close before the start as a withdrawal. Carried on `Case.closed`, it feeds the closed lists, the history and the Atom feed alike. On the 2026-09-23 release with the clock pinned: the county closed lists go from 5,782 events to 8,947 (April 0 to 358, May 0 to 1,153, June 0 to 1,323, July 2,294 to 2,519, August 2,186 to 2,094, September 1,302 to 1,500); history events closed with no date fall from 5,074 to 1,911; 6,773 close dates change, 327 of them to a different month. No availability, person-hour, grade, median or open-list figure moves.
