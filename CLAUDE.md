...
| An incomplete window does not stand in `event_windows`' vote - it is refused wherever it is read, so it must not beat a sibling's complete one. The event **key** survives with a `None` value: it is also the recurrence signal, and dropping it would downgrade a nightly restriction to an outage. | data-quality.md - "A pin can report half a window" (2026-09-13) |
