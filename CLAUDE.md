...
| rules-v2: "until midnight on D" after a start earlier on D ends at D+1 00:00, after a start from 18:00 the day before at D 00:00, and otherwise the rules abstain. An `until` in a sentence about an alternative supply makes the rules abstain. An English completion below an Irish one of the same day is read; Irish alone still abstains. The LLM prompt still reads midnight as the start of D (issue #102). | rules-vs-llm-end-times.md - "rules-v2" (2026-09-24) |
