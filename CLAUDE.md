...
| The county-page link sits under the county heading, not in the footer, and the overview row's `href` points at `c/<county>.html` rather than the hash. Wording is the same sentence esb uses, because the two pages stand in the same relation to their views; it is deliberately not "every notice ever recorded", which the 60-notice cap contradicts. lifts' names the address instead, because its page is the same content as its view. **The area view has no equivalent — accepted gap, closed only by building area pages.** | frontend-notes.md — "The county-page link came up out of the footer" (2026-08-26) |
