...
| rules-v2: "until midnight on D" after a start earlier on D ends at D+1 00:00, after a start the day before at D 00:00, and with no start the rules abstain. An `until` in a sentence about an alternative supply is never the works' end. An English completion below an Irish one is read; Irish alone still abstains. The LLM prompt still reads midnight as the start of D (roadmap). | rules-vs-llm-end-times.md - "rules-v2" (2026-09-24) |
